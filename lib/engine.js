...
function Engine(config) {
  ...
  config.logger = new kademlia.Logger(config.logLevel);

  this.dht = new kademlia.Node(config);

  config.seeds.forEach(function(seed) {
    self.dht.connect(seed);
  });
}
...
Engine.prototype.resolve = function(hash, callback, emitter) {
  ...
  if (!Egg.isEgg(hash)) {
    _resolveChunk(hash, function(err, egg) {
      if (err) {
        return callback(err);
      }
